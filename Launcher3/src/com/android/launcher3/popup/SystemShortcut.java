...
public abstract class SystemShortcut<T extends ActivityContext> extends ItemInfo
    ...
    /**
     * Factory that surfaces a "Remove" option in the popup context menu for apps
     * that live in the hotseat (dock). Only shown for hotseat icons.
     */
    public static final Factory<Launcher> REMOVE_FROM_HOTSEAT =
            (launcher, itemInfo, originalView) -> {
                if (itemInfo == null
                        || itemInfo.container != LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
                    return null;
                }
                return new RemoveFromHotseat(launcher, itemInfo, originalView);
            };

    private static class RemoveFromHotseat extends SystemShortcut<Launcher> {

        RemoveFromHotseat(Launcher target, ItemInfo itemInfo, @NonNull View originalView) {
        ...
        public void onClick(View view) {
            dismissTaskMenuView();
            mTarget.removeItem(mOriginalView, mItemInfo, /* deleteFromDb= */ true,
                    "removed from hotseat via popup menu");
            mTarget.getWorkspace().stripEmptyScreens();
            mTarget.getHotseat().updateHotseatIconScale();
        }
    }
}
