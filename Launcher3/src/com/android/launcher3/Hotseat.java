...
public class Hotseat extends CellLayout implements Insettable {
    ...
    /**
     * Scales all icon views in the hotseat so they evenly fill the available space.
     * When fewer icons are present they grow larger (up to 1.5x); a full dock restores
     * the default scale of 1.0.
     */
    public void updateHotseatIconScale() {
        ShortcutAndWidgetContainer container = getShortcutsAndWidgets();
        final int maxIcons = mActivity.getDeviceProfile().numShownHotseatIcons;
        // Count actual icon views (ignore QSB and gone children)
        int count = 0;
        ...
        if (count == 0) return;
        final float scale = Math.min((float) maxIcons / count, 1.5f);
        for (int i = 0, n = container.getChildCount(); i < n; i++) {
            View child = container.getChildAt(i);
            if (child != null && child.getVisibility() != View.GONE) {
